...
function validatePrimitiveString(): boolean {
  ...
  try {
    const rustData = readBytes('/home/rafael/Development/runar-rust/target/serializer-vectors/prim_string.bin');
    const tsData = readBytes('/home/rafael/Development/runar-ts/target/serializer-vectors-ts/prim_string.bin');
...
function validatePrimitiveBool(): boolean {
  ...
  try {
    const rustData = readBytes('/home/rafael/Development/runar-rust/target/serializer-vectors/prim_bool.bin');
    const tsData = readBytes('/home/rafael/Development/runar-ts/target/serializer-vectors-ts/prim_bool.bin');
...
function validatePrimitiveI64(): boolean {
  ...
  try {
    const rustData = readBytes('/home/rafael/Development/runar-rust/target/serializer-vectors/prim_i64.bin');
    const tsData = readBytes('/home/rafael/Development/runar-ts/target/serializer-vectors-ts/prim_i64.bin');
...
function validatePrimitiveU64(): boolean {
  ...
  try {
    const rustData = readBytes('/home/rafael/Development/runar-rust/target/serializer-vectors/prim_u64.bin');
    const tsData = readBytes('/home/rafael/Development/runar-ts/target/serializer-vectors-ts/prim_u64.bin');
...
function validateBytes(): boolean {
  ...
  try {
    const rustData = readBytes('/home/rafael/Development/runar-rust/target/serializer-vectors/bytes.bin');
    const tsData = readBytes('/home/rafael/Development/runar-ts/target/serializer-vectors-ts/bytes.bin');
    ...
    if (rustBytes.value.length !== tsBytes.value.length) {
      console.log(`❌ Lengths don't match: Rust=${rustBytes.value.length}, TS=${tsBytes.value.length}`);
      return false;
    }

    for (let i = 0; i < rustBytes.value.length; i++) {
      if (rustBytes.value[i] !== tsBytes.value[i]) {
        console.log(`❌ Bytes don't match at index ${i}: Rust=${rustBytes.value[i]}, TS=${tsBytes.value[i]}`);
        return false;
      }
    }
...
function validateJson(): boolean {
  ...
  try {
    const rustData = readBytes('/home/rafael/Development/runar-rust/target/serializer-vectors/json.bin');
    const tsData = readBytes('/home/rafael/Development/runar-ts/target/serializer-vectors-ts/json.bin');
    ...
    if (JSON.stringify(rustJson.value) !== JSON.stringify(tsJson.value)) {
      console.log(`❌ JSON doesn't match: Rust=${JSON.stringify(rustJson.value)}, TS=${JSON.stringify(tsJson.value)}`);
      return false;
    }
...
function validateListAny(): boolean {
  ...
  try {
    const rustData = readBytes('/home/rafael/Development/runar-rust/target/serializer-vectors/list_any.bin');
    const tsData = readBytes('/home/rafael/Development/runar-ts/target/serializer-vectors-ts/list_any.bin');
    ...
    if (rustList.value.length !== tsList.value.length) {
      console.log(`❌ List lengths don't match: Rust=${rustList.value.length}, TS=${tsList.value.length}`);
      return false;
    }

    for (let i = 0; i < rustList.value.length; i++) {
      if (JSON.stringify(rustList.value[i]) !== JSON.stringify(tsList.value[i])) {
        console.log(`❌ List elements don't match at index ${i}: Rust=${JSON.stringify(rustList.value[i])}, TS=${JSON.stringify(tsList.value[i])}`);
        return false;
      }
    }

    console.log(`✅ Heterogeneous list validation passed: [${rustList.value.map(v => JSON.stringify(v)).join(', ')}]`);
    return true;
  } catch (error) {
...
function validateMapAny(): boolean {
  ...
  try {
    const rustData = readBytes('/home/rafael/Development/runar-rust/target/serializer-vectors/map_any.bin');
    const tsData = readBytes('/home/rafael/Development/runar-ts/target/serializer-vectors-ts/map_any.bin');
    ...
    if (JSON.stringify(rustMap.value) !== JSON.stringify(tsMap.value)) {
      console.log(`❌ Maps don't match: Rust=${JSON.stringify(rustMap.value)}, TS=${JSON.stringify(tsMap.value)}`);
      return false;
    }
...
function validateListI64(): boolean {
  ...
  try {
    const rustData = readBytes('/home/rafael/Development/runar-rust/target/serializer-vectors/list_i64.bin');
    const tsData = readBytes('/home/rafael/Development/runar-ts/target/serializer-vectors-ts/list_i64.bin');
    ...
    if (rustList.value.length !== tsList.value.length) {
      console.log(`❌ i64 list lengths don't match: Rust=${rustList.value.length}, TS=${tsList.value.length}`);
      return false;
    }

    for (let i = 0; i < rustList.value.length; i++) {
      if (rustList.value[i] !== tsList.value[i]) {
        console.log(`❌ i64 list elements don't match at index ${i}: Rust=${rustList.value[i]}, TS=${tsList.value[i]}`);
        return false;
      }
    }
...
function validateMapStringI64(): boolean {
  ...
  try {
    const rustData = readBytes('/home/rafael/Development/runar-rust/target/serializer-vectors/map_string_i64.bin');
    const tsData = readBytes('/home/rafael/Development/runar-ts/target/serializer-vectors-ts/map_string_i64.bin');
    ...
    if (JSON.stringify(rustMap.value) !== JSON.stringify(tsMap.value)) {
      console.log(`❌ String->i64 maps don't match: Rust=${JSON.stringify(rustMap.value)}, TS=${JSON.stringify(tsMap.value)}`);
      return false;
    }
...
function validateStructPlain(): boolean {
  ...
  try {
    const rustData = readBytes('/home/rafael/Development/runar-rust/target/serializer-vectors/struct_plain.bin');
    const tsData = readBytes('/home/rafael/Development/runar-ts/target/serializer-vectors-ts/struct_plain.bin');
    ...
    if (rustStruct.value.id !== tsStruct.value.id || rustStruct.value.name !== tsStruct.value.name) {
      console.log(`❌ Structs don't match: Rust={id: "${rustStruct.value.id}", name: "${rustStruct.value.name}"}, TS={id: "${tsStruct.value.id}", name: "${tsStruct.value.name}"}`);
      return false;
    }

    console.log(`✅ Plain struct validation passed: {id: "${rustStruct.value.id}", name: "${rustStruct.value.name}"}`);
    return true;
  } catch (error) {
